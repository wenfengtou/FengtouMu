...
describe("电阻上拉/下拉语义", () => {
  it("GPIO 经电阻接 3V3 时识别为上拉", () => {
    ...
    const nl = buildNetlist(d);
    expect(netPullLevel(nl, { part: "esp", pin: "GPIO2" })).toBe("pullup");
  });
  ...
  it("GPIO 经电阻接 GND 时识别为下拉", () => {
    ...
    const nl = buildNetlist(d);
    expect(netPullLevel(nl, { part: "esp", pin: "GPIO2" })).toBe("pulldown");
  });
  ...
  it("电阻分压节点（上拉+下拉）识别为 divider", () => {
    const d = diagram(
      [
        ...
        { from: { part: "r1", pin: "b" }, to: { part: "esp", pin: "GPIO34" } },
        { from: { part: "r2", pin: "a" }, to: { part: "esp", pin: "GPIO34" } },
        { from: { part: "r2", pin: "b" }, to: { part: "esp", pin: "GND.2" } },
      ],
      [board, { ...resistor, id: "r1" }, { ...resistor, id: "r2" }],
    );
    const nl = buildNetlist(d);
    expect(netPullLevel(nl, { part: "esp", pin: "GPIO34" })).toBe("divider");
  });

  it("直接接 3V3（无电阻）不算上拉语义", () => {
    const d = diagram(
      [{ from: { part: "esp", pin: "GPIO2" }, to: { part: "esp", pin: "3V3" } }],
      [board],
    );
    const nl = buildNetlist(d);
    expect(netPullLevel(nl, { part: "esp", pin: "GPIO2" })).toBeNull();
  });
});
